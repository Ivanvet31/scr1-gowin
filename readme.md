...
* **`src/top/scr1_top_gowin.sv` (Added)**: Instantiates the single SCR1 AHB core, implements an AHB-Lite to SRAM bridge, and maps the CP866 LCD driver to address `0xFF100000`.
...
### Software (C/Assembly)
* **`sw/main.c` (Added)**: Contains the bare-metal C application that interacts with the memory-mapped LCD driver to draw text on the screen.
...
1. **Setup the Toolchain:**
   Ensure you have a 32-bit RISC-V embedded toolchain installed (e.g., `riscv32-none-elf-gcc`).
...
3. **Synthesize the FPGA Design:**
   ...
   * Make sure the generated `firmware.hex` is located in the path specified within `top/scr1_dp_memory.sv`.
